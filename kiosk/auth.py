...
def register():
    if request.method == 'POST':
        ...
        if not username:
            ...
            'SELECT ID FROM MANAGER WHERE LOGIN_ID = ?', (username,)
        ).fetchone() is not None:
            error = 'User {} is already registered.'.format(username)

        if error is None:
            db.execute(
                'INSERT INTO MANAGER (LOGIN_ID, LOGIN_PWD) VALUES (?, ?)',
...
def login():
    if request.method == 'POST':
        ...
        error = None
        user = db.execute(
            'SELECT * FROM MANAGER WHERE LOGIN_ID = ?', (username,)
        ).fetchone()

        if user is None:
            error = 'Incorrect username.'
        elif not check_password_hash(user['LOGIN_PWD'], password):
